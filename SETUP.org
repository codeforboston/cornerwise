...
** Easy Start (OS X or Linux)
  ...
  - The easy start script will try by default to remap ports from the
    virtual machine to localhost, so you should be able to browser to
...
**** Initial Setup
     ...
     - ~docker-machine create -d virtualbox dev~
       ...
       Creates a new machine with the name 'dev'
...
*** Run
    ...
    - ~docker run -it -v $PWD/server:/app -v $PWD/client:/client -v $PWD/data:/data APP_PORT=3000 APP_NAME=cornerwise -p "3000:3000" bdsand/cornerwise /bin/bash~
      ...
      machine. Sets APP_PORT and APP_NAME environment variables, which
      are used by the Django start script. Runs ~/bin/bash~ within the
      new container.
